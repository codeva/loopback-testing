...
module.exports.Resource = function(app, baseUrl){
  ...
  this.accessShouldBeAllowed = function(resourceId, done) {
    var _this = this;
    this.readAccessShouldBeAllowed(resourceId, function(){
      _this.writeAccessShouldBeAllowed(resourceId, done);
    });
  };
  ...
  this.readAccessShouldBeAllowed = function(resourceId, done) {
    this.readAccessShouldReturnStatus(200, resourceId, done);
  };

  this.writeAccessShouldBeAllowed = function(resourceId, done) {
    this.writeAccessShouldReturnStatus(200, resourceId, done);
  };
